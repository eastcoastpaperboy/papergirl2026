...
export function drawPlayer(ctx, game, assets) {
  ...
  if (assets.animated && assets.animFrames && assets.animFrames.length > 0) {
    ...
    let animIndex = neutralIndex;
    if (isTurning) {
      animIndex = Math.floor(animTime * assets.animFps) % assets.animFrames.length;
    } else {
      const nearCenter = [
      ...
      animIndex = nearCenter[Math.floor(animTime * straightFps) % nearCenter.length];
    }
